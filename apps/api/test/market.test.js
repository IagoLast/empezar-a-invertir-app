...
const fixture = () => ({ quoteType: 'EQUITY', symbol: 'AAPL', currency: 'USD', regularMarketPrice: 319.97, regularMarketTime: new Date(now - 20 * 60000), regularMarketChangePercent: -2.51, marketState: 'REGULAR', logoUrl: 'https://s.yimg.com/lb/brands/150x150_apple.png' });
...
test('educational trades accept delayed prices and expose actual timestamps', () => {
  ...
  assert.equal(q.source, 'Yahoo Finance'); assert.equal(q.logoURL, fixture().logoUrl);
});
...
test('logos are optional and restricted to the Yahoo HTTPS image host', () => {
...
test('fundamentals preserve missing values rather than inventing zero', () => {
  assert.deepEqual(normalizeFundamentals({...fixture(),trailingPE:32,epsTrailingTwelveMonths:8},'AAPL',now), {available:true,symbol:'AAPL',pe:32,eps:8,period:'TTM',source:'Yahoo Finance',fetchedAt:new Date(now).toISOString()});
...
test('shared cache prevents Yahoo requests for fifteen minutes', async () => {
...
test('provider failure preserves stale timestamps and does not save a fake fresh quote', async () => {
  ...
  assert.deepEqual(await get('AAPL'),quote);assert.equal(writes,0);
});
test('refresh lease prevents duplicate provider calls and cold-cache failure is explicit', async () => {
  const get=createQuoteService({database:async()=>({quote:null,refresh:false}),loadQuote:async()=>{assert.fail('must not call Yahoo');}});
