...
const routes = new Set(['search', 'market-preview', 'history', 'state', 'trade', 'quote', 'fundamentals', 'lesson', 'revenuecat', 'account']);
createServer(async (req, res) => {
  ...
  try {
    ...
    const request = new Request(`http://localhost:${port}${req.url}`, { method: req.method, headers: req.headers, ...(['GET', 'HEAD'].includes(req.method) ? {} : { body: raw }) });
    const response = await handler(request);
    res.writeHead(response.status, Object.fromEntries(response.headers)); res.end(await response.text());
  } catch { res.writeHead(500); res.end(); }
}).listen(port, '127.0.0.1', () => console.log(`Empezar API: http://localhost:${port}`));
