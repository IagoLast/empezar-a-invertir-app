...
Quote resolution attempts Finnhub and then configured Alpha Vantage for every listing, including international symbols. Each attempt must return the exact requested symbol, explicit currency, supported asset type and valid recent price before it can succeed. Provider symbols are never rewritten to a different listing. Foreign Finnhub quotes require an exact profile ticker and explicit currency; US market status is not applied to foreign exchanges. Historical data also falls back after empty or malformed responses, and preserves the successful provider's currency and resolution. If all attempts fail, the app receives an aggregate availability message rather than one provider's subscription error.

Search ranking uses exact ticker, base ticker, exact company name and name prefix, preserving source order for equal relevance. The app ranks after merging local and remote results. No exchange or country receives a ranking preference.
